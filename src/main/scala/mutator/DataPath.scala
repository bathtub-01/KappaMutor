...
class DataPath extends Module {
  ...
  val heap = Module(new DualPortBlkBoxMem(heapSize, new Application))
  ...
  when(stmReg === StmState.idle && io.progInject) {
    heap.writeA(io.progInjectIO, heapBumper)
  ...
  when(stmReg === StmState.working) {
    ...
    when(
      updateStk.io.elms > 0.U && arity(
        reductionStk.io.top(0)
      ) > reductionStk.io.elms - updateStk.io.top.stackDepth
    ) {
      needUpdate := true.B
      stuckAll := needWrite || (arity(
        reductionStk.io.top(0)
      ) > reductionStk.io.elms - updateStk.io.top.previousStackDepth)
      when(needWrite) {
        ...
        heap.writeB(toWrite, updateStk.io.top.heapAddr)
        updateStk.pop
    ...
    switch(reductionStk.io.top(0).atomType) {
      is(AtomType.PTR) { // assume: heap data is ready for read
        when(!needWrite) { // heap update never happens here
          val app = heap.readOutA
          reductionStk.io.pop := 1.U
          reductionStk.io.push := atomCount(app.app)
          reductionStk.io.din.zip(app.app).foreach { case (port, a) =>
            port := a
          }
      ...
      is(AtomType.COM) {
        val comPayload: ComPayload =
          reductionStk.io.top(0).payload.asTypeOf(new ComPayload)
        ...
        def stackUpdate(): Unit = {
          ...
          reductionStk.io.din.zip(decoder.spine).foreach { case (port, e) =>
            port := translates(e)
          }
        }
        ...
        when( /*!needUpdate*/ !stuckAll) {
          when(app1Valid === 0.U) {
            ...
            // spine + app1
            stackUpdate()
            heap.writeB(translate(decoder.app1), heapBumper)
            heapBumper := heapBumper + 1.U
            preFetch(reductionStk.io.din(0).payload)
            // =========== profiling ports =============
            io.combRed := true.B
            ...
            stackUpdate()
            heap.writeA(translate(decoder.app1), heapBumper)
            heap.writeB(translate(decoder.app2), heapBumper + 1.U)
      ...
      is(AtomType.INT) {
        val prmPayload: PrmPayload =
          reductionStk.io.top(1).payload.asTypeOf(new PrmPayload)
        when( /*!needUpdate*/ !stuckAll) {
      ...
      is(
        AtomType.PRM
      ) { /* PRM won't be showing at stack top -- or maybe it will?*/
      ...
      is(AtomType.Y) {
        when(needWrite || needUpdate) {
          ...
          val toWrite = WireInit(0.U.asTypeOf(new Application))
          toWrite.app(0) := reductionStk.io.top(1)
          toWrite.app(1).atomType := AtomType.PTR
          toWrite.app(1).payload := heapBumper
          heap.writeB(toWrite, heapBumper)
          preFetch(reductionStk.io.din(0).payload)
          heapBumper := heapBumper + 1.U
          // =========== profiling ports =============
          io.yRed := true.B
  ...
  when(
    needWrite
  ) { // second-stage logic, maintianance reading is not handled here
    heap.writeB(pipelineReg, heapBumper)
    heapBumper := heapBumper + 1.U
    needWrite := false.B
    // =========== profiling ports =============
    io.combRed_ := true.B
